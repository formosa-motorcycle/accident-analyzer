    ...
    static async parseCSV(filename, verify = false) {
        ...
        const parseLineBefore2016 = (data) => {
            ...
            if (party.order === 1 && Number(data['主要肇因']) !== party.cause) {
                throw new Error(`${filename}:${lineNumber} The cause of the first party is not the same as the main cause.`);
            }
            ...
            if ((currentCase.date !== prevCase?.date || currentCase.location !== prevCase?.location)
                && party.order !== 1) {
                throw new Error(`${filename}:${lineNumber} The new case doesn't start with the first party.`);
            }
